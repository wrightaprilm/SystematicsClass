...
### [Materials here](https://revbayes.github.io/tutorials/partition/)

This tutorial comes with a recorded video walkthrough, available here: [Walkthrough playlist](https://youtu.be/LPPYGUP1FZc). The video corresponding to each section of the exercise is linked next to the section title.


[Walkthrough video](https://youtu.be/LPPYGUP1FZc)

This tutorial provides the second protocol from our recent publication [@Hoehna2017a].
...
phylogeny and other parameters [@Brown2007].
...
Ronquist2003), in which the sequence alignment is first parsed into a
...
[@Brandley2005, Nylander2004, Mcguire2007]. 
...
[![Walkthrough video]{: height="36" width="36"}](https://youtu.be/LPPYGUP1FZc#t=2m48s)
...
First, clear RevBayes of any extra junk in the proverbial trunk:
...
function.
```{rb}
data_cytb <- readDiscreteCharacterData("primates_and_galeopterus_cytb.nex")
data_cox2 <- readDiscreteCharacterData("primates_and_galeopterus_cox2.nex")
```
...
($\alpha=\beta$) and thus with mean 1.0 [@Yang1994a . Since we do not
...
exponential prior distributions, as done in the The CTMC lesson.
...
saveRev("scripts/mcmc_Unpartitioned.Rev", use_quit=TRUE)
callRevFromTerminal("scripts/mcmc_Unpartitioned.Rev")
```


# Partitioning by Gene Region
[![Walkthrough video]{: height="36" width="36"}](https://youtu.be/LPPYGUP1FZc#t=17m06s)
...
```{r, eval=FALSE}
saveRev("scripts/mcmc_Region.Rev", use_quit=TRUE)
callRevFromTerminal("scripts/mcmc_Region.Rev")
```
...
```
nd the per-partition substitution rate multipliers
```{rb}
# specify a rate multiplier for each partition
part_rate_mult ~ dnDirichlet( rep(1.0, n_data_subsets) )
moves.append( mvBetaSimplex(part_rate_mult, alpha=1.0, tune=true, weight=n_data_subsets) )
...
```{r, eval=FALSE}
saveRev("scripts/mcmc_Gene.Rev", use_quit=TRUE)
callRevFromTerminal("scripts/mcmc_Gene.Rev")
```
...
[![Walkthrough video]{: height="36" width="36"}](https://youtu.be/LPPYGUP1FZc#t=25m37s)
...
# Exercises 
[![Walkthrough video]{: height="36" width="36"}](https://youtu.be/LPPYGUP1FZc#t=29m11s)
...
3.  **Partitioned model selection.** Bayes factors are
    computed as the ratio of marginal likelihoods (see [here](https://revbayes.github.io/tutorials/model_selection_bayes_factors/bf_intro.html) for more details). Rather than constructing the analysis with an
