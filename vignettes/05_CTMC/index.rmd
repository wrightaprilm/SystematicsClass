...
This tutorial demonstrates how to set up and perform analyses
...
Specific functions for substitution models available in RevBayes.
 ...
 |:----------------:|:---------------------------:|:---------------:|:-----------------------:|
 |   Jukes-Cantor   |    @Jukes1969    |      fnJC       |           -             |
 | K80 (a.k.a. K2P) |    @cite Kimura1980    |      fnK80      |        $\kappa$         |
 |  Felsenstein-81  | @Felsenstein1981 |      fnF81      |         $\pi$           |
 |        T92       |    @Tamura1992   |      fnT92      | $\pi_{GC}$, $\kappa$    |
 |        HKY       |   @Hasegawa1985  |      fnHKY      |     $\pi$, $\kappa$     |
 |        GTR       |    @Tavare1986   |      fnGTR      |     $\pi$, $\epsilon$   |




# Character Evolution under the Jukes-Cantor Substitution Model 
...
(1) Jukes Cantor model and the (2) branch length models.
...
In @jc_graphical_model, the tree topology is denoted $\Psi$ and the 
...
Some studies, *e.g.* Brown2010 Rannala2012, 
...
the corresponding branch lengths @Zhang2012.
...
Alternative Prior on Time-Trees: Tree Topology and Node Ages
...
Here we simply assumed that the tree is 10.0 time units old. We could also specify a prior on the root age if we have fossil calibrations. Next, we specify the `tree` stochastic variable by passing in the taxon information `taxa` to the `dnUniformTimeTree()` distribution:
...
move (`mvFNPR`) and its Metropolized-Gibbs variant (`mvGPR`) @Hoehna2008 Hoehna2012. 
...
For more information on molecular clocks please read the 
...
Thus, it is important to use programs like [Tracer](http://tree.bio.ed.ac.uk/software/tracer/) @Rambaut2011to evaluate mixing and non-convergence.
...
![JC](../figures/primates_cytb_JC_TL_Trace.png)
...
![map](../figures/primates_cytb_JC_tree.png) width="800" 
...
The K80 model (AKA the K2P model) allows the rates of transition and transversion substitutions to be unequal @Kimura1980. 
...
corresponds to the substitution model proposed by @Hasegawa1985,
...
-   With @jc_graphical_modelas your guide, draw the probabilistic
...
-   Like the HKY model, the Felsenstein 1981 (F81) substitution model
    ...
    transition-transversion rates @Felsenstein1981. 
    Can you set up the F81 model and run an analysis?

-   Complete the @tab_primates_posteriorby reporting the posterior
...
reversible (GTR) substitution model @Tavare1986, which allows all six
exchangeability rates to differ (@gtr_graphical_model).
...
The node `pi` represents the $\pi$ node in @gtr_graphical_model. Now add the simplex scale move on the stationary frequencies to the moves vector:
...
This approach for accommodating ASRV is another example of a hierarchical model (@fig_gtrg). 
...
Here we describe an extension to our phylogenetic model to accommodate invariable sites. Under the invariable-sites model @Hasegawa1985, each site is invariable with probability `p_inv`, and variable with probability $1-$`p_inv`.
