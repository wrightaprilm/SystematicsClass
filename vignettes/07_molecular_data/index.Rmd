...
This tutorial comes with a recorded video walkthrough. The video corresponding to each section of the exercise is linked next to the section title. The full playlist is available here: ![Walkthrough playlist](https://www.youtube.com/playlist?list=PLztACvN0g42t9pLJpeUel-ynfuNdWOsa4)


Additionally, we will be using the R package Revticulate to run RevBayes from RStudio.
...
This will open the Renviron file. You will add the following to it:

```
rb=/cloud/project/software/rb
```

# Overview 
[![Walkthrough video](https://youtu.be/x4ADIyqbYGM)

This tutorial covers the first protocol from [@Hoehna2017a]
...
![Walkthrough video](https://youtu.be/z-94P0d10us)
##  Getting Started 
...
If you download this file and place it in a directory called `scripts` inside your main tutorial directory,you can easily execute this analysis using the `source()` function in the RevBayes console:
...
##  Loading the Data 
![Walkthrough video](https://youtu.be/kU-6WEv0mRQ)
...
setwd("vignettes/07_molecular_data")
```
...
```
```
/Users/tombayes/vignettes/07_molecular_data
```
...
![Walkthrough video](https://youtu.be/S1XTkUOWTLo)
...
In the Jukes-Cantor graphical model, the tree topology is denoted $\Psi$ and the
...
We can plot this tree, too, in R:

```{r}

topology <- doRev("topology")

phytools::plotTree(topology)
...
>Tree Topology and Node Ages: Alternatively, you may want to specify a prior on time-trees.
...
>Fore more information on tree priors, such as birth-death processes, we'll discuss this more in the timetree tutorials.
...
![Walkthrough video](https://youtu.be/XaXJ26rD16c)
...
This times out. So what we will do is save our Rev commands to a file and run them via the command `callRevFromTerminal()`. 

```{r, echo=TRUE, eval=FALSE}
saveRev("mcmc_JC.Rev", use_wd = TRUE, use_quit = TRUE)
callRevFromTerminal("mcmc_JC.Rev")
```
...
![Walkthrough video](https://youtu.be/ZJhj8wR9YNs)
...
![Walkthrough video](https://youtu.be/cqeyZyjIcuw)
...
-   With JC graphical model as your guide, draw the probabilistic
...
![Walkthrough video](https://youtu.be/mqLv0INyit4)
...
![Walkthrough video](https://youtu.be/JN1nOT7iSOE)
...
![Walkthrough video](https://youtu.be/0P4yLk0jxps)
